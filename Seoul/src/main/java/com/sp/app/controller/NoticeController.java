...
public class NoticeController {
	...
	public ResponseEntity<?> download(
		...
		return ResponseEntity
				.status(HttpStatus.FOUND)//302 상태코드(리다이렉트)
				.location(URI.create(redirectUrl))//Location 헤더에 리다이렉트할 url설정
				.build();
	...
	public ResponseEntity<?> zipdownload(@PathVariable(name = "num") long num,
		...
		try {
			...
			if(listFile.size() > 0) {
				...
				String zopFilename = fileName.substring(0, fileName.lastIndexOf(".")) + "_외.zip";
				...
				return storageService.downloadZipFile(source, originals, zopFilename);
			}
		} catch (Exception e) {
			log.info("zipdownload : ", e);
		}
		
		String redirectUrl = req.getContextPath() + "/notice/downlioadFailed";
		return ResponseEntity
